...
class ProjectViewsTestCase(TestCase):
    def setUp(self):
        ...
        self.membership1 = Membership.objects.create(
            user_profile=self.user1_profile,
            organization=self.org1,
            role='admin'
        )
        self.membership2 = Membership.objects.create(
            user_profile=self.user2_profile,
    ...
    def test_project_create_with_valid_data(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertTrue(Project.objects.filter(name='New Project', organization=self.org1).exists())
    ...
    def test_project_edit_access_control(self):
        ...
        self.client.login(username='user1', password='testpass123')
        response = self.client.post(reverse('projects:edit', args=[self.project2.id]), {
            'name': 'Hacked Project'
        })
    ...
    def test_project_edit_updates_name(self):
        ...
        self.client.login(username='user1', password='testpass123')
        response = self.client.post(reverse('projects:edit', args=[self.project1.id]), {
...
class ApiKeyViewsTestCase(TestCase):
    ...
    def test_api_key_create_requires_authentication(self):
        ...
        response = self.client.post(reverse('projects:key_create', args=[self.project.id]), {
            'name': 'Test Key'
        })
        self.assertEqual(response.status_code, 302)
    ...
    def test_api_key_create_access_control(self):
        ...
        self.client.login(username='user1', password='testpass123')
        response = self.client.post(reverse('projects:key_create', args=[project2.id]), {
    ...
    def test_api_key_created_shows_raw_key_once(self):
        ...
        response = self.client.get(reverse('projects:key_created', args=[self.project.id, api_key.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_api_key_created_requires_authentication(self):
        ...
        api_key = ApiKey.objects.create(name='Test Key', project=self.project, hashed_key='test')
        response = self.client.get(reverse('projects:key_created', args=[self.project.id, api_key.id]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_api_key_revoke_requires_post(self):
        ...
        self.client.login(username='user1', password='testpass123')
        api_key = ApiKey.objects.create(name='Test Key', project=self.project, hashed_key='test')
        response = self.client.get(reverse('projects:key_revoke', args=[self.project.id, api_key.id]))
        self.assertEqual(response.status_code, 405)

    def test_api_key_revoke_requires_authentication(self):
        """Test that API key revoke redirects unauthenticated users."""
        api_key = ApiKey.objects.create(name='Test Key', project=self.project, hashed_key='test')
        response = self.client.post(reverse('projects:key_revoke', args=[self.project.id, api_key.id]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_api_key_revoke_access_control(self):
        ...
        org2 = Organization.objects.create(name='Org 2')
        project2 = Project.objects.create(name='Project 2', organization=org2)
        api_key = ApiKey.objects.create(name='Test Key', project=project2, hashed_key='test')
        
        self.client.login(username='user1', password='testpass123')
        response = self.client.post(reverse('projects:key_revoke', args=[project2.id, api_key.id]))
        self.assertEqual(response.status_code, 302)  # Redirects with error
